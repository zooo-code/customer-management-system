...
- 만약에 가장 인기 있는 상품을 불러온다고 생각해보자.
  - 장바구니에 많이 담긴 기준?
  - 주문이 가장 많이 들어온 기준?
  - 따로 인기 상품을 관리하는 테이블을 만들어서 관리?
  - 따로 도메인을 만든다?
  - 사실 매번 쿼리로 불러와서 조회한 다음에 하는 방법은 안좋은 방법 같다.
  - 아니면 필드를 추가해서 진행해야 할까? 만약에 현재 운영중인 서비스라면 어떻게 해야할까?
  - 어떤 쇼핑몰을 기획 할때 판매 상품에 대한 카운트를 해두면 좋다는 것을 알게 되었다.
  - 만약에 운영중인 서비스에 다른 필드를 추가할때는 어떤 방식으로 업데이트를 할까?

- 양방향 연관 관계 점검
  - 카트, 카트 아이템 보니 제대로 둘다 넣어줬다.